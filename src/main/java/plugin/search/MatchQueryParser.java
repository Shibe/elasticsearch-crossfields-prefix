...
public class MatchQueryParser {
    ...
    public enum Type implements Writeable {
        ...
        BOOLEAN_PREFIX(3),
        /**
         * The text is analyzed, terms are added to a boolean query with all terms acting as a prefix.
         */
        BOOLEAN_ALL_PREFIX(4);
    ...
    class MatchQueryBuilder extends QueryBuilder {
        ...
        protected Query createBooleanALlPrefixQuery(String field, String queryText, BooleanClause.Occur occur) {
            return createQuery(field, queryText, Type.BOOLEAN_ALL_PREFIX, occur, 0);
        }
            ...
            try (CachingTokenFilter stream = new CachingTokenFilter(source)) {
                ...
                if (numTokens == 0) {
                    ...
                    if (positionCount == 1) {
                        ...
                        if (type == Type.BOOLEAN_ALL_PREFIX) {
                            return analyzeMultiBoolean(field, stream, operator, Type.BOOLEAN_ALL_PREFIX);
                        }
                        return analyzeMultiBoolean(field, stream, operator, type);
                    }
                }
            } catch (IOException e) {
        ...
        private Query analyzeMultiBoolean(
                String field,
                TokenStream stream,
                BooleanClause.Occur operator,
                Type type
            ...
            while (stream.incrementToken()) {
                if (posIncrAtt.getPositionIncrement() != 0) {
                    add(q, field, currentQuery, operator, isPrefix(type, lastOffset, -1));
                    currentQuery.clear();
                }
            ...
            stream.end();
            add(q, field, currentQuery, operator, isPrefix(type, lastOffset, offsetAtt.endOffset()));
            return q.build();
        }

        private boolean isPrefix(Type type, int lastOffset, int endOffset) {
            return switch (type) {
                case BOOLEAN_ALL_PREFIX -> true;
                case BOOLEAN_PREFIX -> lastOffset == endOffset;
                default -> false;
            };
        }
