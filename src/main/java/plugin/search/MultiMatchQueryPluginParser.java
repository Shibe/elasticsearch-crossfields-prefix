...
public class MultiMatchQueryPluginParser extends MatchQueryParser {

    private Float groupTieBreaker = null;

    public MultiMatchQueryPluginParser(SearchExecutionContext context) {
        super(context);
    }
    ...
    public Query parse(MultiMatchQueryPluginBuilder.Type type, Map<String, Float> fieldNames, Object value, String minimumShouldMatch)
            throws IOException {
        ...
        switch (type) {
            ...
            case CROSS_FIELDS:
                queries = buildCrossFieldQuery(fieldNames, value, minimumShouldMatch, tieBreaker, false);
                break;

            case CROSS_FIELDS_PREFIX:
                queries = buildCrossFieldQuery(fieldNames, value, minimumShouldMatch, tieBreaker, true);
                break;
            default:
    ...
    private List<Query> buildFieldQueries(
            MultiMatchQueryPluginBuilder.Type type,
    ...
    private List<Query> buildCrossFieldQuery(
        ...
        for (Map.Entry<Analyzer, List<FieldAndBoost>> group : groups.entrySet()) {
            ...
            Query query;
            if (prefix) {
                query = builder.createBooleanALlPrefixQuery(representativeField, value.toString(), occur);
            } else {
                query = builder.createBooleanQuery(representativeField, value.toString(), occur);
            }
            if (query == null) {
    ...
    static Query blendTerms(
        ...
        for (FieldAndBoost ft : blendedFields) {
            for (BytesRef term : values) {
                ...
                try {
                    if (prefix) {
                        query = ft.fieldType.prefixQuery(new String(term.bytes), null, context);
                    } else {
